...
interface SalesReportProps {
  ...
  stores?: AdminTypes.ReportTypes.Store[];
  category?: AdminTypes.ReportTypes.Category[];
  products?: AdminTypes.ReportTypes.Product[];
}

export default function SalesReportPage({ initialData = [], stores = [], category = [], products = [] }: SalesReportProps) {
  ...
  const uniqueStores = React.useMemo(() => {
    return stores.map(store => ({ label: store.name, value: store._id }));
  }, [stores]);

  const uniqueCategories = React.useMemo(() => {
    return category.map(cat => ({ label: cat.categoryName, value: cat.categoryName }));
  }, [category]);

  const uniqueProducts = React.useMemo(() => {
    let filteredProducts = products;
    if (filterValues.category && filterValues.category !== "All") {
      filteredProducts = products.filter(p => p.category.categoryName === filterValues.category);
    }

    // Flatten products to include variants as separate entries
    const options: { label: string; value: string }[] = [];

    filteredProducts.forEach(prod => {
      if (prod.hasVariation && prod.variantData && prod.variantData.length > 0) {
        prod.variantData.forEach(variant => {
          const variantName = variant.variantValues.map(v => v.value).join(" - ");
          const name = `${prod.productName} - ${variantName}`;
          options.push({ label: name, value: name });
        });
      } else {
        options.push({ label: prod.productName, value: prod.productName });
      }
    });

    return options;
  }, [products, filterValues.category]);
  ...
  ], [uniqueCategories, uniqueProducts, uniquePaymentMethods]);
