...
interface LoyaltyTabProps {
    loyaltyPoints: AdminTypes.loyaltyTypes.LoyaltyPointServerResponse[];
    pagination: PaginationType.Pagination;
    onViewDetails: (item: AdminTypes.loyaltyTypes.LoyaltyPointServerResponse) => void;
    onEdit: (item: AdminTypes.loyaltyTypes.LoyaltyPointServerResponse) => void;
    onDelete: (id: string) => void;
    onToggleStatus: (row: AdminTypes.loyaltyTypes.LoyaltyPointServerResponse, next: boolean) => Promise<void>;
}
...
export default function LoyaltyTab({
    ...
    const [selectedRows, setSelectedRows] = React.useState<AdminTypes.loyaltyTypes.LoyaltyPointServerResponse[]>([]);
    const [actionFilter, setActionFilter] = React.useState<string>("All");
    const [activeStatusFilter, setActiveStatusFilter] = React.useState<string>("All");
    const [downloadData, setDownloadData] = React.useState<AdminTypes.loyaltyTypes.LoyaltyPointServerResponse[]>([]);
    const [clearSelectedRows, setClearSelectedRows] = React.useState(false);

    const { searchTerm, setSearchTerm, statusFilter, setStatusFilter, filteredData } = customHooks.useListFilters<AdminTypes.loyaltyTypes.LoyaltyPointServerResponse>(
        loyaltyPoints,
    );
    ...
    const handleToggleStatusById = React.useCallback(
        async (id: string, next: boolean) => {
            const row = loyaltyPoints.find(l => l._id === id);
            if (row) {
                await onToggleStatus(row, next);
            }
        },
        [loyaltyPoints, onToggleStatus]
    );

    const tableColumns = React.useMemo(() => WebComponents.WebCommonComponents.CommonComponents.createColumns<AdminTypes.loyaltyTypes.LoyaltyPointServerResponse>({
        fields: [
            {
                name: "Loyalty Points",
                selector: (row) => row.loyaltyPoints.toString(),
                sortable: true
            },
            {
                name: "Minimum Amount",
                selector: (row) => row.minimumAmount.toString(),
                sortable: true
            },
            {
                name: "Maximum Amount",
                selector: (row) => row.maximumAmount?.toString() || "-",
                sortable: true
            },
            {
                name: "Expiry Duration (Days)",
                selector: (row) => (row.expiryDuration?.toString() || "-"),
                sortable: true
            },
        ],
        status: {
    ...
    const exportColumns = React.useMemo(() => {
        return WebComponents.WebCommonComponents.CommonComponents.generateExportColumns<AdminTypes.loyaltyTypes.LoyaltyPointServerResponse>([
            {
                key: "loyaltyPoints",
                label: "Loyalty Points",
                accessor: (row) => row.loyaltyPoints?.toString() || '-',
                pdfWidth: 30
            },
            {
                key: "minimumAmount",
                label: "Minimum Amount",
                accessor: (row) => row.minimumAmount?.toString() || '-',
                pdfWidth: 30
            },
            {
                key: "maximumAmount",
                label: "Maximum Amount",
                accessor: (row) => row.maximumAmount?.toString() || '-',
                pdfWidth: 30
            },
            {
                key: "expiryDuration",
                label: "Expiry Duration (Days)",
                accessor: (row) => row.expiryDuration?.toString() || '-',
                pdfWidth: 30
            },
            {
                key: "status",
    ...
    const downloadPdfData = async (): Promise<any[]> => {
        ...
        if (selectedRowsIds.length > 0) {
            res = await ServerActions.ServerActionslib.bulkGetSelectedLoyaltyPointsAction({ ids: selectedRowsIds });
        } else {
            res = await ServerActions.ServerActionslib.bulkGetLoyaltyPointsAction(filterDatas);
        }
    ...
    const handleBulkApply = React.useCallback(async () => {
        ...
        if (actionFilter !== 'Status') {
            if (actionFilter === 'Delete') {
                ...
                try {
                    const result = await ServerActions.ServerActionslib.bulkDeleteLoyaltyPointsAction({ ids });
                    if (!result?.success) throw new Error(result?.error || 'Failed to delete selected loyalty points');
                    WebComponents.UiComponents.UiWebComponents.SwalHelper.success({ text: 'Selected loyalty points deleted.' });
                    clearSelectedData()
                } catch (error: unknown) {
                    const errorMessage = error instanceof Error ? error.message : 'Failed to delete selected loyalty points.';
        ...
        try {
            const result = await ServerActions.ServerActionslib.bulkUpdateLoyaltyPointsStatusAction({ ids, status: isActive });
    ...
    return (
        <div className="bg-white dark:bg-darkFilterbar rounded-[4px] h-full w-full mt-4 overflow-hidden">
                ...
                renderExports={
                    <>
                        <WebComponents.UiComponents.UiWebComponents.DownloadCSV
                            ...
                            filename={`loyalty-points-${new Date().toISOString().split('T')[0]}.csv`}
                            onExport={async () => {
                        ...
                        <WebComponents.UiComponents.UiWebComponents.DownloadPDF
                            ...
                            filename={`loyalty-points-${new Date().toISOString().split('T')[0]}.pdf`}
            ...
            <WebComponents.WebCommonComponents.CommonComponents.DataTable
                ...
                onSelectedRowsChange={({ selectedRows }) => setSelectedRows(selectedRows as AdminTypes.loyaltyTypes.LoyaltyPointServerResponse[])}
