        ...
        <>
            ...
            {!selectedRole && !showAddRoleForm && (
                <div className="flex flex-col gap-4">
                    {Constants.rolemodules.ROLE_DEFINITIONS.filter(role => role.key !== 'admin').map((role) => (
            ...
            {selectedRole && (
                <div className="bg-white dark:bg-[#1F1F1F] rounded-lg shadow-sm w-full">
                    <div className="sticky top-16 z-20 bg-white dark:bg-[#1F1F1F] rounded-t-lg shadow-sm">
                        <div className="p-4 border-b border-gray-100 dark:border-[#2F2F2F] flex justify-end gap-3">
                            <button
                                onClick={handleClear}
                                className="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded text-sm font-medium flex items-center gap-2 transition-colors"
                            >
                                {Constants.adminConstants.clear}
                            </button>
                            <button
                                onClick={handleSave}
