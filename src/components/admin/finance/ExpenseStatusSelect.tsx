...
const ExpenseStatusSelect = ({
    ...
    // Approved/Rejected -> final, no further options
    // const getAvailableOptions = (status: 'Approved' | 'Pending' | 'Rejected'): Array<'Approved' | 'Pending' | 'Rejected'> => {
    const getAvailableOptions = (status: string): Array<'approved' | 'pending' | 'rejected'> => {
        const statusValu = status
        switch (statusValu) {
            case 'pending':
    ...
    const toggleDropdown = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (!isOpen && buttonRef.current) {
            ...
            setPosition({
                top: rect.bottom + 5,
                left: rect.left + rect.width / 2 - 72, // center align for w-36 (144px)
            });
        }
        setIsOpen(prev => !prev);
    };
    ...
    return (
        <>
            ...
            {isOpen && !isDisabled && typeof document !== 'undefined' &&
                createPortal(
                    <>
                        ...
                        >
                            {availableOptions.map(option => (
                                ...
                                >
                                    <span
                                        className={`w-2 h-2 rounded-full ${option === 'pending'
                                            ? 'bg-yellow-500'
                                            : option === 'approved'
                                                ? 'bg-blue-500'
                                                : 'bg-red-500'
