...
export default function ExpenseCategoryManagement({
  ...
  const [editingCategory, setEditingCategory] = React.useState<AdminTypes.ExpenseTypes.Expense.ExpenseCategory | null>(null);
  const [selectedCategory, setSelectedCategory] = React.useState<AdminTypes.ExpenseTypes.Expense.ExpenseCategory | null>(null);
  const [actionFilter, setActionFilter] = React.useState("All");
  const [activeStatusFilter, setActiveStatusFilter] = React.useState("All");
  const [selectedRows, setSelectedRows] = React.useState<AdminTypes.ExpenseTypes.Expense.ExpenseCategory[]>([]);
  const [clearSelectedRows, setClearSelectedRows] = React.useState(false);
  const [downloadData, setDownloadData] = React.useState([])
  const router = useRouter();
  const searchParams = useSearchParams();
  const { searchTerm, setSearchTerm, statusFilter, setStatusFilter, filteredData } = customHooks.useListFilters<AdminTypes.ExpenseTypes.Expense.ExpenseCategory>(
    categories,
  );
  ...
  const bulkActionOptions = React.useMemo(() => {
    ...
    return options.filter(option => {
      if (option.value === 'Status') {
        return checkPermission("finance.expensecategory", "update");
      }
      if (option.value === 'Delete') {
        return checkPermission("finance.expensecategory", "delete");
      }
      return true;
    });
  }, [checkPermission]);
  ...
  const exportColumns = React.useMemo(() => {
    return WebComponents.WebCommonComponents.CommonComponents.generateExportColumns<AdminTypes.ExpenseTypes.Expense.ExpenseCategory>([
      {
        key: 'name',
  ...
  const handleAdd = async (formData: any) => {
    await ServerActions.HandleFunction.handleAddCommon({
      formData,
      createAction: ServerActions.ServerActionslib.createExpenseCategoryAction,
      setLoading,
      setShowModal,
      router,
      ...
      onSuccess: (result) => {
        if (result?.data?.data) {
          setCategories(prev => [result.data.data, ...prev]);
        }
      },
    });
  };
  ...
  const handleEdit = async (formData: any) => {
    await ServerActions.HandleFunction.handleEditCommon({
      ...
      updateAction: (id: string | number, data: any) =>
        ServerActions.ServerActionslib.updateExpenseCategoryAction(id as string, data),
      setLoading,
      setShowEditModal,
  ...
  // View Details
  const handleViewDetails = React.useCallback((category: AdminTypes.ExpenseTypes.Expense.ExpenseCategory) => {
    setSelectedCategory(category);
    setShowDetailsModal(true);
  }, []);

  const handleToggleStatus = React.useCallback(async (row: AdminTypes.ExpenseTypes.Expense.ExpenseCategory, next: boolean) => {
    setCategories(prev => prev.map(category => (category._id === row._id ? { ...category, status: next } : category)));
    await ServerActions.HandleFunction.handleToggleStatusCommon({
      row,
      next,
      getId: (item) => item._id,
      preparePayload: () => ({ name: row.name, description: row.description, status: next }),
      updateAction: (id: string | number, data: any) =>
        ServerActions.ServerActionslib.updateExpenseCategoryAction(id as string, data),
      setLoading,
      router,
  ...
  const handleDelete = async (id: string) => {
    await ServerActions.HandleFunction.handleDeleteCommon({
      ...
      deleteAction: (id: string | number) =>
        ServerActions.ServerActionslib.deleteExpenseCategoryAction(id as string),
      setLoading,
      router,
  ...
  // Open edit modal
  const handleEditModal = React.useCallback((category: AdminTypes.ExpenseTypes.Expense.ExpenseCategory) => {
  ...
  const handleBulkApply = React.useCallback(async () => {
    await ServerActions.HandleFunction.handleBulkApplyCommon({
      ...
      bulkDeleteAction: ServerActions.ServerActionslib.bulkDeleteExpenseCategoriesAction,
      bulkUpdateStatusAction: ServerActions.ServerActionslib.bulkUpdateExpenseCategoriesStatusAction,
      clearSelectedData,
      idSelector: (r) => r._id,
      statusProp: 'status',
    });
  ...
  const downloadPdf = async (): Promise<any[]> => {
    return await ServerActions.HandleFunction.handleDownloadCommon({
      selectedRows,
      searchParams,
      bulkGetSelectedAction: ServerActions.ServerActionslib.bulkGetSelectedExpenseCategoryAction,
      bulkGetAllAction: ServerActions.ServerActionslib.bulkGetExpenseCategoryAction,
      setDownloadData,
    });
  };

  const tableColumns = React.useMemo(() => WebComponents.WebCommonComponents.CommonComponents.createColumns<AdminTypes.ExpenseTypes.Expense.ExpenseCategory>({
    fields: [
      {
        name: Constants.adminConstants.categoryName,
        selector: (row: AdminTypes.ExpenseTypes.Expense.ExpenseCategory) => row.name,
        sortable: true,
        width: "20%"
      },
      {
        name: Constants.adminConstants.descriptionColumn,
        selector: (row: AdminTypes.ExpenseTypes.Expense.ExpenseCategory) => row.description || '-',
        sortable: true,
        width: "30%"
      },
      {
        name: Constants.adminConstants.createdOn,
        selector: (row: AdminTypes.ExpenseTypes.Expense.ExpenseCategory) => {
    ...
    actions:
      [
        ...
        {
          render: (row) => (
            (checkPermission("finance.expensecategory", "update")) && (
        ...
        {
          render: (row) => (
            (checkPermission("finance.expensecategory", "delete")) && (
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        <div className="flex-1 min-w-0">
          <h1 className="text-lg sm:text-lg md:text-xl lg:text-2xl font-medium text-textMain dark:text-white truncate">
            {Constants.adminConstants.expenseCategories}
            {(() => {
          ...
          <p className="text-xs sm:text-sm md:text-base text-textSmall pt-2">
            {Constants.adminConstants.manageExpenseCategoriesForOrganizingExpensesLabel}
          </p>
        </div>
        {(checkPermission("finance.expensecategory", "create")) && (
          <WebComponents.UiComponents.UiWebComponents.Button
            ...
            renderExports={
              <>
                <WebComponents.UiComponents.UiWebComponents.DownloadCSV
                  ...
                  filename={`expense-categories-${new Date().toISOString().split('T')[0]}.csv`}
                  onExport={async () => {
      ...
      {(showModal || showEditModal) && (
        ...
        >
          {editingCategory ? (
            <WebComponents.AdminComponents.AdminWebComponents.Forms.ExpenseCategoryForm
              onSubmit={handleEdit}
              category={editingCategory}
            />
          ) : (
            <WebComponents.AdminComponents.AdminWebComponents.Forms.ExpenseCategoryForm onSubmit={handleAdd} />
          )}
        </WebComponents.UiComponents.UiWebComponents.AdminFormModal>
      )}
      ...
      {showDetailsModal && selectedCategory && (
        <WebComponents.AdminComponents.AdminWebComponents.Models.ExpenseCategoryDetailsModal
