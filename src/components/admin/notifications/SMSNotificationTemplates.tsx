...
export default function SMSNotificationTemplates({
    initialTemplates,
    initialPagination
}: {
    readonly initialTemplates: AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate[];
    readonly initialPagination: PaginationType.Pagination;
}) {
    const router = useRouter();
    const searchParams = useSearchParams();
    const pathname = usePathname();
    const [templates, setTemplates] = React.useState<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate[]>(initialTemplates);
    const [pagination, setPagination] = React.useState<PaginationType.Pagination>(initialPagination);
    const [showModal, setShowModal] = React.useState(false);
    const [showEditModal, setShowEditModal] = React.useState(false);
    const [editingTemplate, setEditingTemplate] = React.useState<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate | null>(null);
    const [loading, setLoading] = React.useState(false);
    const [selectedRows, setSelectedRows] = React.useState<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate[]>([]);
    const [actionFilter, setActionFilter] = React.useState<string>("All");
    const [activeStatusFilter, setActiveStatusFilter] = React.useState<string>("All");
    const [downloadData, setDownloadData] = React.useState<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate[]>([]);
    const [clearSelectedRows, setClearSelectedRows] = React.useState(false);
    const [activeField, setActiveField] = React.useState<'smsContent' | null>(null);
    const [selectedUserType, setSelectedUserType] = React.useState<string>('');
    const [isEditing, setIsEditing] = React.useState(false);
    const { searchTerm, setSearchTerm, statusFilter, setStatusFilter, filteredData } = customHooks.useListFilters<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate>(
        templates,
    )

    const smsEditorRef = React.useRef<RichTextEditorRef>(null);
    ...
    const handleParameterClick = (param: string) => {
        ...
        if (activeField === 'smsContent') {
            if (smsEditorRef.current) {
                smsEditorRef.current.insertContent(placeholder);
            }
        }
    };

    const handleEditModal = React.useCallback((template: AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate) => {
    ...
    const handleSaveTemplate = async () => {
        ...
        try {
            // Assuming there is a createSMSTemplateAction - reusing mail action name if strictly typed or using any if not available yet on client side lib
            const result = await ServerActions.ServerActionslib.createSMSTemplateAction(editingTemplate as any);

            if (result.success) {
                WebComponents.UiComponents.UiWebComponents.SwalHelper.success({ text: 'SMS template saved successfully!' });
                setIsEditing(false);
                setShowEditModal(false);
                setEditingTemplate(null);
    ...
    const handleBulkApply = React.useCallback(async () => {
        ...
        try {
            const result = await Promise.all(ids.map(id => ServerActions.ServerActionslib.toggleSMSTemplateStatusAction(id)));
    ...
    const handleToggleStatus = React.useCallback(async (row: AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate, next: boolean) => {
        await ServerActions.HandleFunction.handleToggleStatusCommon({
            ...
            updateAction: (id: string | number, data) => ServerActions.ServerActionslib.toggleSMSTemplateStatusAction(String(id)),
            setLoading,
            router,
    ...
    const tableColumns = React.useMemo(() => WebComponents.WebCommonComponents.CommonComponents.createColumns<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate>({
        fields: [
            {
                name: "Template Label",
                selector: (row: AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate) => row.label,
                sortable: true
            },
            {
                name: "Type",
                selector: (row: AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate) => row.type,
                sortable: true
            },
            {
                name: "To",
                selector: (row: AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate) => Array.isArray(row.to) ? row.to.join(', ') : (row.to || '-'),
                sortable: true
            },
        ],
        status: {
    ...
    const exportColumns = React.useMemo(() => {
        return WebComponents.WebCommonComponents.CommonComponents.generateExportColumns<AdminTypes.NotificationTypes.SMSNotificationTypes.SMSNotificationTemplate>([
            {
                key: "label",
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-2">
                <div>
                    <h1 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">
                        {showEditModal ? `SMS Notification Templates > Edit Template` : 'SMS Notification Templates'}
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-300">Manage and customize SMS notification templates for your system</p>
                </div>
            </div>
                            ...
                            renderExports={
                                <>
                                    <WebComponents.UiComponents.UiWebComponents.DownloadCSV
                                        ...
                                        filename={`sms-templates-${new Date().toISOString().split('T')[0]}.csv`}
                                        onExport={async () => {
                                    ...
                                    <WebComponents.UiComponents.UiWebComponents.DownloadPDF
                                        ...
                                        filename={`sms-templates-${new Date().toISOString().split('T')[0]}.pdf`}
                                        title="SMS Templates Report"
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <WebComponents.UiComponents.UiWebComponents.RichTextEditor
                                                key={`sms-${editingTemplate._id || 'new'}-${selectedUserType}`}
                                                ref={smsEditorRef}
                                                value={editingTemplate.templates?.find(t => t.userType === selectedUserType)?.sms?.content || ''}
                                                onChange={(val) => {
                                                ...
                                                onFocus={() => setActiveField('smsContent')}
