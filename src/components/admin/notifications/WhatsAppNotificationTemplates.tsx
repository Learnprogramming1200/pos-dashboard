...
export default function WhatsAppNotificationTemplates({
    initialTemplates,
    initialPagination
}: {
    readonly initialTemplates: AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate[];
    readonly initialPagination: PaginationType.Pagination;
}) {
    const router = useRouter();
    const searchParams = useSearchParams();
    const pathname = usePathname();
    const [templates, setTemplates] = React.useState<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate[]>(initialTemplates);
    const [pagination, setPagination] = React.useState<PaginationType.Pagination>(initialPagination);
    const [showModal, setShowModal] = React.useState(false);
    const [showEditModal, setShowEditModal] = React.useState(false);
    const [editingTemplate, setEditingTemplate] = React.useState<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate | null>(null);
    const [loading, setLoading] = React.useState(false);
    const [selectedRows, setSelectedRows] = React.useState<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate[]>([]);
    const [actionFilter, setActionFilter] = React.useState<string>("All");
    const [activeStatusFilter, setActiveStatusFilter] = React.useState<string>("All");
    const [downloadData, setDownloadData] = React.useState<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate[]>([]);
    const [clearSelectedRows, setClearSelectedRows] = React.useState(false);
    const [activeField, setActiveField] = React.useState<'whatsappContent' | null>(null);
    const [selectedUserType, setSelectedUserType] = React.useState<string>('');
    const [isEditing, setIsEditing] = React.useState(false);
    const { searchTerm, setSearchTerm, statusFilter, setStatusFilter, filteredData } = customHooks.useListFilters<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate>(
        templates,
    )

    const whatsappEditorRef = React.useRef<RichTextEditorRef>(null);
    ...
    const handleParameterClick = (param: string) => {
        ...
        if (activeField === 'whatsappContent') {
            if (whatsappEditorRef.current) {
                whatsappEditorRef.current.insertContent(placeholder);
            }
        }
    };

    const handleEditModal = React.useCallback((template: AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate) => {
    ...
    const handleSaveTemplate = async () => {
        ...
        try {
            const result = await ServerActions.ServerActionslib.createWhatsAppTemplateAction(editingTemplate as any);

            if (result.success) {
                WebComponents.UiComponents.UiWebComponents.SwalHelper.success({ text: 'WhatsApp template saved successfully!' });
                setIsEditing(false);
                setShowEditModal(false);
                setEditingTemplate(null);
                ...
                WebComponents.UiComponents.UiWebComponents.SwalHelper.error({ text: result.error || 'Failed to save template' });
            }
        } catch (error: any) {
            WebComponents.UiComponents.UiWebComponents.SwalHelper.error({ text: error.message || 'Failed to save template' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleBulkApply = React.useCallback(async () => {
        ...
        try {
            const result = await Promise.all(ids.map(id => ServerActions.ServerActionslib.toggleWhatsAppTemplateStatusAction(id)));
    ...
    const handleToggleStatus = React.useCallback(async (row: AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate, next: boolean) => {
        await ServerActions.HandleFunction.handleToggleStatusCommon({
            ...
            updateAction: (id: string | number, data) => ServerActions.ServerActionslib.toggleWhatsAppTemplateStatusAction(String(id)),
            setLoading,
            router,
    ...
    const tableColumns = React.useMemo(() => WebComponents.WebCommonComponents.CommonComponents.createColumns<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate>({
        fields: [
            {
                name: "Template Label",
                selector: (row: AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate) => row.label,
                sortable: true
            },
            {
                name: "Type",
                selector: (row: AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate) => row.type,
                sortable: true
            },
            {
                name: "To",
                selector: (row: AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate) => Array.isArray(row.to) ? row.to.join(', ') : (row.to || '-'),
                sortable: true
            },
        ],
        status: {
    ...
    const exportColumns = React.useMemo(() => {
        return WebComponents.WebCommonComponents.CommonComponents.generateExportColumns<AdminTypes.NotificationTypes.WhatsAppNotificationTypes.WhatsAppNotificationTemplate>([
            {
                key: "label",
    ...
    const downloadPdf = async (): Promise<any[]> => {
        ...
        // Replace with WhatsApp bulk get
        let res = { data: { data: { data: selectedRows.length > 0 ? selectedRows : templates } } };
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-2">
                <div>
                    <h1 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">
                        {showEditModal ? `WhatsApp Notification Templates > Edit Template` : 'WhatsApp Notification Templates'}
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-300">Manage and customize WhatsApp notification templates for your system</p>
                </div>
            </div>
                            ...
                            renderExports={
                                <>
                                    <WebComponents.UiComponents.UiWebComponents.DownloadCSV
                                        ...
                                        filename={`whatsapp-templates-${new Date().toISOString().split('T')[0]}.csv`}
                                        onExport={async () => {
                                    ...
                                    <WebComponents.UiComponents.UiWebComponents.DownloadPDF
                                        ...
                                        filename={`whatsapp-templates-${new Date().toISOString().split('T')[0]}.pdf`}
                                        title="WhatsApp Templates Report"
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <WebComponents.UiComponents.UiWebComponents.RichTextEditor
                                                key={`whatsapp-${editingTemplate._id || 'new'}-${selectedUserType}`}
                                                ref={whatsappEditorRef}
                                                value={editingTemplate.templates?.find(t => t.userType === selectedUserType)?.whatsapp?.content || ''}
                                                onChange={(val) => {
                                                ...
                                                onFocus={() => setActiveField('whatsappContent')}
                                                placeholder="Enter WhatsApp template content..."
                                                height={400}
                                            />
                                        </div>
                                    </div>
