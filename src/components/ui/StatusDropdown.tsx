...
export type StatusDropdownProps = React.ComponentProps<typeof Dropdown> & {
  className?: string;
...
export const StatusDropdown: React.FC<StatusDropdownProps> = ({ className, placeholder, options, optionLabel, optionValue, pt, panelClassName, ...props }) => {
  const computedOptions = React.useMemo(() => {
    const base = options && Array.isArray(options) && options.length > 0 ? options : DEFAULT_STATUS_OPTIONS;
    // Normalize to PrimeReact format if consumer passes custom objects
    return base.map((opt) => ({ label: opt.label, value: opt.value }));
  }, [options]);
  ...
  return (
    <Dropdown
      ...
      optionLabel={optionLabel ?? ("name" as any)}
      optionValue={optionValue ?? ("value" as any)}
      pt={mergedPt}
