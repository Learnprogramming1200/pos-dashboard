...
export default function Sidebar({ collapsed, toggleCollapse }: SidebarProps) {
  ...
  // Use cached settings from context instead of fetching again (performance optimization)
  const { generalSettings } = useGeneralSettings();
  const settings = generalSettings as GeneralSettingsData | null;

  // Use cached permissions from auth store (performance optimization - no duplicate API calls)
  const authStorePermissions = useAuthStore((state) => state.permissions);
  ...
  const isActive = React.useCallback(
    (href: string) => {
      ...
      // Root dashboard path is a prefix for all dashboard pages.
      // To avoid highlighting both "Dashboard" and the current submenu,
      // treat it as active ONLY on the exact /dashboard route.
      if (path === "/dashboard") {
  ...
  // Use permissions from auth store (already cached by AuthProvider - no duplicate fetch)
  const permissions = authStorePermissions;
    ...
    () => [
      ...
      },
      {
        ...
        dropdownItems: [
          ...
          {
            label: "SMS Settings",
            href: "/admin/system-settings/sms-setting",
            icon: FaSms,
            permissionKey: "settings.sms",
          },
          {
            label: "WhatsApp Settings",
            href: "/admin/system-settings/whatsapp-setting",
            icon: FaWhatsapp,
      ...
      },
      {
        ...
        href: "/admin/helpdesk/support",
        icon: FaHeadset,
        permissionKey: "helpdesk.support",
      },
    ],
    [],
  );
  ...
  // Cashier menu items - now use /dashboard paths (unified namespace)
  // Permission keys are kept the same as they match the API response structure
  const cashierItems: MenuItem[] = React.useMemo(() => {
    // Transform the /admin prefix to /dashboard for unified routing
    return transformMenuItems(adminItems, "/admin", "/dashboard");
  }, [adminItems, transformMenuItems]);

  // Manager menu items - now use /dashboard paths (unified namespace)
  const managerItems: MenuItem[] = React.useMemo(() => {
    // Transform the /admin prefix to /dashboard for unified routing
    return transformMenuItems(adminItems, "/admin", "/dashboard");
  }, [adminItems, transformMenuItems]);

  // Dashboard items for admin role - same /dashboard paths
  const dashboardItems: MenuItem[] = React.useMemo(() => {
    // Transform the /admin prefix to /dashboard for unified routing
    return transformMenuItems(adminItems, "/admin", "/dashboard");
  }, [adminItems, transformMenuItems]);
  ...
  const menuItems: MenuItem[] = React.useMemo(() => {
    ...
    if (userRole === "superadmin") {
      ...
      // Cashier uses dashboard items with permission filtering
      items = cashierItems;
    } else if (userRole === "manager") {
      // Manager uses dashboard items with permission filtering
      items = managerItems;
    } else if (userRole === "admin") {
      // Admin now also uses dashboard items (unified routing)
      items = dashboardItems;
    } else {
      // Fallback to dashboard items
      items = dashboardItems;
    }
    ...
    return items;
  }, [userRole, superadminItems, adminItems, cashierItems, managerItems, dashboardItems, permissions, checkPermission]);
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex-shrink-0 border-b-[0.4px] border-[#CCCDCD]">
        <Link href="/" title="Back to Home" prefetch={false}>
          <Image
            src={logoSrc}
